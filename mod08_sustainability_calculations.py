...
def compute_energy_kwh(jobs_df, power_per_core_kw=POWER_PER_CORE_KW):
    ...
    df["energy_kwh"] = ( 
        ...
        #product of the CPU Cores, Runtime hours, Kilowatts used per 
        # core

        #in the instructions section fo the notebook, generate_data
        #was imported as 
        # "from mod08_generate_emissions_data import generate_data"
        #then gave the jobs df to the notebook, to be passed in here!

        #above the coder make a copy of that just called df
        #whihc is where I got the df from, much like the df["energy_kwh"]
        #also above
